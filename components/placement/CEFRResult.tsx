  ...
  if (!result) {
    return (
      <div className="text-center py-16">
        ...
        <Link
          href="/test"
          className="mt-4 inline-block text-[var(--gold)] hover:underline text-[0.88rem]"
        >
          Take the test →
        </Link>
      </div>
  ...
  return (
    <div>
      ...
      <div className="bg-[var(--ink-2)] border border-[rgba(201,168,76,0.3)] rounded-sm p-10 text-center mb-8 relative overflow-hidden">
        ...
        />
        <p className="text-[0.7rem] tracking-[0.2em] uppercase text-[var(--gold)] mb-4">
          Your CEFR Level
        </p>
        <div
          className="font-light text-[6rem] text-[var(--gold)] leading-none mb-2"
      ...
      <div className="bg-[var(--ink-2)] border border-[rgba(245,240,232,0.07)] rounded-sm p-6 mb-8">
        <h3
          className="font-semibold text-[1.1rem] mb-4"
          style={{ fontFamily: "'Cormorant Garamond', serif" }}
        >
          Score by Level
        </h3>
        <div className="flex flex-col gap-3">
          {Object.entries(scoreByLevel).map(([lvl, score]) => (
            <div key={lvl}>
      ...
      {recommendedCourse && (
        <div className="bg-[var(--ink-2)] border border-[rgba(201,168,76,0.2)] rounded-sm p-6">
          <p className="text-[0.7rem] tracking-widest uppercase text-[var(--gold)] mb-2">
            Recommended for You
          </p>
          <h3
            className="font-semibold text-[1.4rem] mb-2"
            style={{ fontFamily: "'Cormorant Garamond', serif" }}
          >
            {recommendedCourse.title}
          </h3>
          <p className="text-[var(--muted)] text-[0.85rem] mb-5">
